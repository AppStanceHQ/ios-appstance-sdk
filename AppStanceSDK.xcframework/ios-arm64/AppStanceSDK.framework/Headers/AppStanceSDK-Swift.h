...
#ifndef APPSTANCESDK_SWIFT_H
#define APPSTANCESDK_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="AppStanceSDK",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_CLASS("_TtC12AppStanceSDK9AppStance")
@interface AppStance : NSObject
...
/// note:
/// This method is safe to call multiple times, but initialization will only occur once per app launch.
...
///
/// \param fBAnonymousID An optional Facebook anonymous ID for attribution purposes.
///
/// \param revenueCatUserID An optional RevenueCat user ID for advanced purchase analytics.
///
...
///
/// \param amount The revenue amount (Double format) .
///
...
///   </li>
///   <li>
///     The event name is empty or has already been tracked on this device.
///   </li>
/// </ul>
/// Example usage:
/// \code
...
/// Example usage:
/// \code
/// let userID = AppStanceSDK.getAppStanceUserID()
/// if !userID.isEmpty {
///     // Use the user ID
/// }
///
/// \endcode
/// returns:
/// The AppStance user ID as a <code>String</code>, or an empty string if unavailable.
+ (NSString * _Nonnull)getAppStanceUserID SWIFT_WARN_UNUSED_RESULT;
/// Retrieves the remote configuration as a JSON string.
/// This function attempts to fetch the <code>remote_config</code> property from the remote SDK configuration
/// s and serializes it to a pretty-printed JSON string. If the property is missing,
/// not a dictionary, serialization fails, or any error occurs, it returns an empty JSON dictionary string (<code>{}</code>).
/// note:
/// Clients should check for the empty dictionary string (<code>{}</code>) and take appropriate steps to handle missing or invalid configuration.
/// Example usage:
/// \code
/// let remoteConfigJSON = AppStanceSDK.getRemoteConfigJSONString()
/// if remoteConfigJSON != "{}" {
